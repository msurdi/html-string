...
describe("Html templates", () => {
  ...
  it("Interpolates simple values", () => {
    ...
    expect(
      template("funky")
        .render()
        .trim()
    ).toEqual("<p>A simple template with a value of funky</p>");
  });
  ...
  it("Renders a falsy value as an empty string except if it is a number", () => {
    ...
    expect(
      template(false)
        .render()
        .trim()
    ).toEqual("<p>A simple template with a value of </p>");
    ...
    expect(
      template(0)
        .render()
        .trim()
    ).toEqual("<p>A simple template with a value of 0</p>");
  });
  ...
  it("Interpolates an object value as its toString() value", () => {
    ...
    expect(
      template(testValue)
        .render()
        .trim()
    ).toEqual("<p>A simple template with a value of Serialized.</p>");
  });
  ...
  it("Escapes dangerous values (xss)", () => {
    ...
    expect(
      template("<script>alert('boom!')</script>")
        .render()
        .trim()
    ).toEqual(
  ...
  it("Does not escape templates passed as values", () => {
    ...
    expect(
      template(templateValue())
        .render()
        .trim()
    ).toEqual(`<div><h1>A value with tags</h1></div>`);
  });
  ...
  it("Does not escape dangerous values with the :safe modifier", () => {
    const template = value =>
      html`
        <p>A simple template with a value of ${value}:safe</p>
      `;
